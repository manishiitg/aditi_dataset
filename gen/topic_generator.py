...
def fetch_deepest_subcategory(category, depth=1, current_depth=0, added_depth=False):
    ...
    try:
        ...
        valid_subcats = [subcat['title'].replace("Category:", "") for subcat in categorymembers if "list" not in subcat['title'].lower(
        ) and "by" not in subcat['title'].lower() and "stubs" not in subcat['title'].lower()]
        ...
        for _ in range(len(valid_subcats)):  # Attempt to select a valid subcategory
            ...
            if current_depth < depth:
                deeper_category = fetch_deepest_subcategory(
                    selected_subcat, depth, current_depth + 1, added_depth)
...
if __name__ == "__main__":
    ...
    for topic in TOPICS:
        ...
        for _ in range(6):  # Repeat to get four deep subcategories
            deepest_subcategory = fetch_deepest_subcategory(
                topic, depth=random.randint(2, 3))
